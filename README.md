...
- The Linux computers at UIUC are managed by EWS
...
- Examples:
    * The path to the download directory is given by:
        * `/home/netid/Downloads`
    * Imagine we have a file named `downloaded_file` inside the download directory. The path to it would be:
...
- Examples with relative paths:
    ...
    * We want to access `file2` in `folder5`
        ...
        * You can also chain `..` together, i.e. `../../file3` if `file3` is inside `folder4`
...
- Important flags:
    * `-p` creates parent directory if it doesn't exist
- Arguments:
...
Try it out:
- `vim ./dir_1/cool_file`
    * You should see "Goodbye!" again.
- There's much you can do with `vim` but let's focus on the basics
    * To begin typing, hit `i` on your keyboard. you should see `INSERT` pop up at the bottom of the terminal
    * Change `Goodbye!` to `Hello!`
    * Hit `Esc` on your keyboard to stop modifying the file
    * Type `:wq` and hit `Enter` on your keyboard to save your changes and exit
    * Now do `vim ./dir_1/cool_file` again. You should see "Hello!", if you followed the steps correctly
    * Now exit vim again. This time, since you have not made any changes, type `:q` instead of `:wq` and hit `Enter`
...
Host github.com
    ...
    IdentitiesOnly Yes
```
7. Change the permission via `chmod 700 ~/.ssh/config`
8. Head to github.com, Navigate to your profile at the top right corner ->Settings->SSH and GPG Keys (under "Access" on the left column)
...
13. Go back to your home directory `~` via `cd`
14. Clone your repo via `git clone git@github.com:illinois-cs-coursework/fa26_ece120_netid.git ece120`
...
### Endpoint setup
1. Add the release endpoint to your repo by doing `git remote add release git@github.com:illinois-cs-coursework/fa26_ece120_.release.git`
...
2. Create a file named `bar.txt` that contains **exactly** the following: `This is my file!`
...
- Git realizes these files have been changed but is ignoring them  
...
- Now you should be able to see the changes on github.com

## Turn-ins
- Your github repo state is your turn-in
- An autograder will be ran on the repo to check if your changes are correct. Review campuswire announcements for details on how the autograder will work.

Below is the expected final directory structure after completing this lab. The order of files within each directory does not matter.
```
lab1
    -stuff
        - baz.txt
